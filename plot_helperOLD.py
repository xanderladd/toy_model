...
# Plot for GA result
def plot_indv_section(axs, param_names,params_dist,sect_name,bar_size):
...
def final_indv_plot_by_section(param_names, final_best_indv, title, file_path_to_save=None, max_xtic=1, vert_size=10):
    dend_inds = range(0,4)
    axon_inds = range(4,12)
    somatic_inds = range(12,19)
    fig_ga = plt.figure(figsize=(cm_to_in(12), cm_to_in(7)))
    fig_ga.subplots_adjust(wspace=0.5)
    gs = fig_ga.add_gridspec(2, 2,height_ratios=[len(somatic_inds),len(dend_inds)])
    ax_soma = fig_ga.add_subplot(gs[0, 0])
    
    ax_dend = fig_ga.add_subplot(gs[1, 0])
    ax_axon = fig_ga.add_subplot(gs[:, 1])
    
    somatic_param_names = [param_names[i] for i in somatic_inds]
    somatic_dist_params = [final_best_indv[i] for i in somatic_inds]
    plot_indv_section(ax_soma,somatic_param_names,somatic_dist_params,'Somatic',0.8)
    ...
    plot_indv_section(ax_dend,dend_param_names,dend_dist_params,'Dendritic',0.8)
...
def plot_stim_volts_pair(stim, volts,times, title_stim, title_volts, file_path_to_save=None):
...
def run_and_plot_voltage_response_single(base, stim_data, file_save_path, stim_title):
    ...
    plot_stim_volts_pair(stim_data, [volts_target], times, stim_title,'', file_save_path)
...
def run_and_plot_voltage_response(base, ga_result, stim_data, file_save_path, stim_title, response_title):
    ...
    plot_stim_volts_pair(stim_data, [volts_target, volts_best_response], times, stim_title, response_title, file_save_path)
